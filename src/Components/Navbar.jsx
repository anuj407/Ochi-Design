...
function Navbar() {
  ...
  return (
    <div className={`nav ${click ?`bg-zinc-950 pb-12`:`backdrop-blur mix-blend-difference`} px-6 w-[100vw] fixed z-50 py-6 md:px-16 flex justify-between items-center font-['Neue_Montreal'] `}>
        ...
        <div className={`absolute z-[999] w-[100vw] ${click ? `translate-y-0`:`-translate-y-[120%]`} duration-1000 left-0 top-[15vh] h-[90vh] bg-zinc-950 md:top-[10vh] pt-[8vw] pl-[8vw] gap-[3vh] flex-col border-t-[1px] border-zinc-200 lg:pl-0 lg:bg-transparent lg:pt-0 lg:border-none lg:flex-row lg:top-[2rem] lg:w-fit lg:h-fit lg:relative flex lg:gap-10 `}>
        {links.map((item,index)=><a  onMouseEnter={()=>handleHover(index)} onMouseLeave={()=>handleLeave(index)} className={`relative w-fit text-[11vw] leading-[10.2vw] h-[11vw] md:text-[9vw] md:leading-[8.4vw] font-extralight lg:leading-[2.9vh] lg:opacity-80 lg:text-[1.07rem] cursor-pointer lg:h-[3vh] overflow-hidden  ${index == 4 &&`lg:ml-[19vw]`} `} key={index}>
          <div   key={index} className={`${item.hover ? `-translate-y-[105%]`:`translate-y-0`} duration-200 w-fit h-full flex flex-col `}>
            <span>{item.name}</span>
            <span>{item.name}</span>       
          </div>
          <span className={`${item.hover && `scale-x-[1] origin-left`} scale-x-0 origin-top-right transition-transform duration-500 w-[105%]  absolute top-[93%]  h-[1.5px] bg-zinc-50`}></span>
          </a>)}
          <div className={` ${click ? `block`:`hidden`} text-[5vw] mt-[10%] flex flex-col justify-evenly`}>
           <p>E:</p> 
           <a href="#">hello@ochi.design</a>
          <div className=" flex flex-col justify-between">
            <p>© ochi design 2024. Legal Terms</p>
            <p className="mt-[10%] -ml-[10%] opacity-60 text-center">Websites By Qbys</p>
          </div>
        </div>
        </div>
        <div onClick={()=>handleClick()} className={`${click ? `hidden`:`block`}  text-[2rem]  lg:hidden`}><TfiMenu/></div>
        <div onClick={()=>handleClick()} className={`${click ? `block`:`hidden`}  text-[2.5rem] ml-4 lg:hidden`}><IoClose/></div>
